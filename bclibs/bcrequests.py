...
from urllib3.exceptions import HTTPError, ProtocolError, ResponseError, \
    RequestError
...
if settings.base_url:
    base_url = Url(scheme="https", host=settings.base_url, path=None).url
    ...
    def get_token(self) -> [MappingProxyType[dict[str, str]], None]:
        ...
        while self.token == "" or self.token_time < datetime.now():
            ...
            if http_result.status != 200 and self.token == "":
                ...
                self.token_time = datetime.now() + timedelta(
                    seconds=float(http_result_dict.get("expires_in"))
...
def do_request(
    ...
    if not updated_logger:
        temp_logger = bclogging.get_parent_logger()
    ...
    except (HTTPError, ProtocolError, ResponseError, urlTimeoutError,
...
def get_bc_lines(
    ...
    **kwargs
    ...
    """
    request_result = ""
    logger.info(f"Retrieving %c.yellow%{line_name}%c% lines")
    ...
    logger.info(f"Finished retrieving %c.yellow%{line_name}%c% lines")
...
def bc_line_menu() -> tuple[list, list, list, list, list, list]:
    ...
    def print_menu(
        ...
        if len(print_menu_options) > 1:
            ...
            if print_menu_title != "Date":
                menu_options_list.append("All")
...
def retrieve_policy_list_user(contact_name: str, check_name: bool = True,
                              **kwargs) -> list:
    """Search for user
...
def new_bc_policy(policy: dict, **kwargs) -> bool:
    """Creates new policy
    :param policy: Policy number
    :type policy: dict
    :return: Request result
    ...
    try:
        policy_exists = (loads(
            request_result.data.decode("utf-8")
            ).get("data").get("exists"))
...
def update_bc_rating_information(
    property_id: str, line: str, limit: int,
    **kwargs
        ) -> list:
    """
...
def retrieve_bc_notes(policy_id: str) -> list:
    ...
    notes_search = {"id": policy_id, "pageSize": 1000, "page": 0,
                    "ascending": False}
    request_result = do_request(
