...
from britecore_sdk.exceptions import NotFoundError, BritecoreError
...
async def fetch_policies_concurrent(
    ...
    # Run concurrently
    print(f"Fetching {len(policy_ids)} policies with max {max_concurrent} concurrent...")
...
async def create_quotes_batch(
    ...
    # Create tasks
    tasks = [
        create_with_limit(idx, data)
        for idx, data in enumerate(quote_data_list)
    ]
    
    # Run concurrently
    print(f"Creating {len(quote_data_list)} quotes with max {max_concurrent} concurrent...")
    results = await asyncio.gather(*tasks)
