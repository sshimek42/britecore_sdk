...
class APIWithRetry:
    ...
    def retrieve_policy_with_fallback(
        self, policy_number: str | None = None, policy_id: str | None = None
    ) -> dict[str, Any] | None:
        """
    ...
    def _retrieve_with_retry(self, func, description: str) -> dict[str, Any]:
        ...
        for attempt in range(self.max_retries + 1):
            try:
                ...
                if attempt > 0:
                    logger.info(
                        f"✓ {description} retrieved after {attempt + 1} attempts"
                    )
                return result
                ...
                if attempt < self.max_retries:
                    ...
                    logger.error(
                        f"Failed {description} after {self.max_retries} retries"
                    )
                    raise
...
def create_quotes_with_error_collection(
    quote_list: list[dict[str, Any]], retry_handler: APIWithRetry
) -> dict[str, Any]:
    """
    ...
    """
    results = {"succeeded": [], "failed": [], "errors_summary": {}}
    ...
    for idx, quote_data in enumerate(quote_list):
        try:
            ...
            results["failed"].append(
                {"index": idx, "error": str(e), "type": "validation"}
            )
            ...
            results["failed"].append(
                {"index": idx, "error": str(e), "type": "rate_limited"}
            )
            ...
            results["failed"].append(
                {"index": idx, "error": str(e), "type": "api_error"}
            )
...
def main():
    ...
    policy = handler.retrieve_policy_with_fallback(
        policy_number="POL-123-456", policy_id="12345"
    )
    if policy:
