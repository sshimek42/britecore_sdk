...
def do_request(
    ...
    """
    if request_headers is None or token_class == "":
        request_headers = {}
    if not request_headers and token_class != "":
    ...
    try:
        if json:
            ...
            if token_class == "":
                json = dumps({"api_key": SITE_SETTINGS.api_key}).encode("utf-8")
            request_result = http.request(
                method=method,
                url=_full_url(path),
                headers=request_headers,
...
def line_menu() -> tuple[list, list, list, list, list, list]:
    ...
    def print_menu(
        ...
        if len(print_menu_options) > 1:
            ...
            if tmp_line in ("All", ""):
                id = list(print_menu_options.values())
                name = list(print_menu_options.keys())
            else:
                id = print_menu_options.get(tmp_line)
                name = tmp_line
        else:
            print("1. " + print_menu_default)
            tmp_line = print_menu_default
            id = print_menu_options.get(menu_default)
            name = menu_default
...
def get_polices_w_line(line: str, **kwargs) -> dict:
...
def add_line(revision: str, line: str, **kwargs) -> bool:
...
def retrieve_policy_ids(policy: str, **kwargs) -> tuple[str, str]:
    ...
    policy_json = get_policy(policy, **kwargs)
...
def get_property_information(property_id: str, **kwargs) -> dict:
...
def retrieve_policy_contact_info(policy: str, **kwargs) -> list:
    ...
    contact_json = get_policy(policy, **kwargs)
...
def add_contact_role(contact_id, role="Named Insured", **kwargs) -> dict:
...
def new_policy(policy: dict, **kwargs) -> tuple[bool, Any]:
    """Creates new policy
    :param policy: Policy number
    :type policy: dict
    :return: Request result
...
def get_policy(policy_number: str, **kwargs) -> dict:
...
def get_policy_terms(policy_id: str, **kwargs) -> list[dict[str, list[dict]]]:
    """
...
def retrieve_revision(revision: str, **kwargs) -> dict:
    """
