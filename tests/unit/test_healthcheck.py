...
from britecore_sdk.utils.healthcheck import (
    HealthcheckResult,
    _format_result,
    main,
    run_healthcheck,
...
def test_run_healthcheck_config_failure() -> None:
    ...
    with patch(
        "britecore_sdk.utils.healthcheck.init_api_client",
...
def test_run_healthcheck_skip_ping_success() -> None:
    ...
    with patch("britecore_sdk.utils.healthcheck.init_api_client") as mock_init:
...
def test_run_healthcheck_ping_failure() -> None:
    ...
    with (
        patch("britecore_sdk.utils.healthcheck.init_api_client") as mock_init,
        patch(
            "britecore_sdk.utils.healthcheck.v2_utils.get_release_info",
...
def test_main_returns_nonzero_on_failure() -> None:
    ...
    with patch(
        "britecore_sdk.utils.healthcheck.run_healthcheck",
